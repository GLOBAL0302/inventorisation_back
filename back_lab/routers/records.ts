...
recordsRouter.post("/", imagesUpload.single("image"), async (req, res) => {
    ...
    const insertResult = await mysqlDb.getConnection().query(
        'INSERT INTO records (category_id, location_id, title, description, image) VALUES (?,?,?,?,?)',
        [newRecord.category_id, newRecord.location_id, newRecord.title, newRecord.description, newRecord.image],
    )
...
recordsRouter.put("/:id", imagesUpload.single("image"), async (req, res) => {
    ...
    const insertResult = await mysqlDb.getConnection().query(
        'UPDATE records SET category_id = ?, location_id = ?, title = ?, description = ?, image = ? WHERE id = ?',
        [newRecord.category_id, newRecord.location_id, newRecord.title, newRecord.description, newRecord.image, newRecord.id],
    )
