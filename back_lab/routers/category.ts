...
categoryRouter.get('/:id', async (req, res) => {
  ...
  const result = await mysqlDb.getConnection().query('SELECT * FROM category WHERE id = ?', [id]);
  const categories = result[0] as ICategory[];
...
categoryRouter.post('/', async (req, res) => {
  ...
  const insertResult = await mysqlDb
    .getConnection()
    .query('INSERT INTO category (title, description) VALUES (?,?)', [newCategory.title, newCategory.description]);

  const resultHeader = insertResult[0] as ResultSetHeader;
  const getNewResult = await mysqlDb
    .getConnection()
    .query('SELECT * FROM category WHERE id = ?', [resultHeader.insertId]);
...
categoryRouter.delete('/:id', async (req, res) => {
  ...
  const result = await mysqlDb.getConnection().query('SELECT * FROM records where category_id = ?', [id]);
  const category_id = result[0] as ICategory[];
  if (category_id[0]) {
    res.status(404).send('can not delete the bound category');
  }
  const deleted = await mysqlDb.getConnection().query('DELETE FROM category where id = ?', [id]);
