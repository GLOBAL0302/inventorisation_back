...
locationRouter.get('/:id', async (req, res) => {
  ...
  const result = await mysqlDb.getConnection().query('SELECT * FROM location WHERE id = ?', [id]);
  const locations = result[0] as ILocation[];
...
locationRouter.post('/', async (req, res) => {
  ...
  const insertResult = await mysqlDb
    .getConnection()
    .query('INSERT INTO location (title, description) VALUES (?,?)', [newLocation.title, newLocation.description]);

  const resultHeader = insertResult[0] as ResultSetHeader;
  const getNewResult = await mysqlDb
    .getConnection()
    .query('SELECT * FROM location WHERE id = ?', [resultHeader.insertId]);
...
locationRouter.delete('/:id', async (req, res) => {
  ...
  const result = await mysqlDb.getConnection().query('SELECT * FROM records where location_id = ?', [id]);
  const location_id = result[0] as ICategory[];
  if (location_id[0]) {
    res.status(404).send('can not delete the bound location');
  }
  const deleted = await mysqlDb.getConnection().query('DELETE FROM location where id = ?', [id]);
...
locationRouter.put('/:id', async (req, res) => {
  ...
  const result = await mysqlDb
    .getConnection()
    .query('UPDATE location SET title = ?, description = ? WHERE id = ?', [
      updates.title,
      updates.description,
      updates.id,
    ]);
